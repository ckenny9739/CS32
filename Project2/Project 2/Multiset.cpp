...
: m_count(0)
{
    m_head = new Node;                                                 // Create dummy node + assign values
...
: m_count(0)                                                        // Incremented via insert member function
{
...
bool Multiset::insert(const ItemType& value)
{
    ...
    {
        if(p->m_data == value)                                      // If node already exists
        {
            p->m_size++;
            return true;
        }
    }
    Node* insert = new Node;                                        // If not - New node
...
int Multiset::erase(const ItemType& value)
{
    for (Node* p = m_head->m_next; p != m_head; p = p->m_next)      // Erase 1 or 0 elements
        ...
        {
            p->m_size--;
            if(p->m_size == 0)                                      // If size == 0, delete node
            {
...
int Multiset::eraseAll(const ItemType& value)
{
    ...
    for (Node* p = m_head->m_next; p != m_head; p = p->m_next)      // Erase all instances of element
        if(p->m_data == value)
        {
            ...
            delete p;
            m_count--;
            return numRemoved;                                      // Need inner return to break from loop
        }
...
int Multiset::get(int i, ItemType& value) const
{
    ...
    for(int j = 0; j < i; j++)                  // Go forward i nodes
        n = n->m_next;
...
void subtract(const Multiset& ms1, const Multiset& ms2, Multiset& result)
{
    ...
    for (int i = 0; i < ms2.uniqueSize(); i++)
    {
        ...
        if(count1 > count2)                                     // result gets count1-count2 of element
            for (int j = 0; j < count2; j++)                    // Erase count2 elements from result
